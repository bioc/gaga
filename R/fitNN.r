fitNN <- function(x, groups, patterns, trace=TRUE) {
  require(EBarrays)
  ...
  ngroup <- table(groupsr)
  hypotheses <- character(nrow(patterns))
  for (i in 1:length(hypotheses)) hypotheses[i] <- paste(rep(patterns[i,]+1,ngroup),collapse=' ')
  hypotheses <- ebPatterns(hypotheses)
  #Fit model
  expx <- exp(x)
  nn.fit <- emfit(data=expx, family="LNNMV", hypotheses=hypotheses, groupid=groupsr, verbose=as.integer(trace))
  ...
  ans <- list(parest=parest, patterns=patterns, pp=pp)
  class(ans) <- 'nnfit'
  return(ans)
}
