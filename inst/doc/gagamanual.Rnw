...
Specifying \texttt{method=='Gibbs'} or \texttt{method=='MH'} fits a fully
...
\texttt{'EM'} and \texttt{'SA'} are computationally quicker than
\texttt{'Gibbs'} and \texttt{'MH'}, but the latter provide credibility intervals
as well as point estimates.
In our experience the four methods usually deliver similar results.

By default the routine uses a Simulated Annealing scheme with 200 iterations
to find hyper-parameter values with high posterior density.
Of course, a larger number of iterations may be specified, but in practice
we have found that good hyper-parameter values can be found for the GaGa model
with only 200 iterations.

<<four>>=
patterns <- matrix(c(0,0,0,1),2,2)
gg1 <- fitGG(x[,c(-6,-12)],groups,patterns=patterns,nclust=1,method='Gibbs',B=1000,trace=FALSE)
gg2 <- fitGG(x[,c(-6,-12)],groups,patterns=patterns,nclust=1,method='MH',B=1000,trace=FALSE)
gg3 <- fitGG(x[,c(-6,-12)],groups,patterns=patterns,nclust=1,method='SA',B=200,trace=FALSE)
gg4 <- fitGG(x[,c(-6,-12)],groups,patterns=patterns,method='EM',trace=FALSE)  
@ 
...
Therefore, simply typing \texttt{plot(gg1\$mcmc)} produces the
...
gg2 <- parest(gg2,x=x[,c(-6,-12)],groups,burnin=100,alpha=.05)
gg3 <- parest(gg3,x=x[,c(-6,-12)],groups,alpha=.05)
gg4 <- parest(gg4,x=x[,c(-6,-12)],groups,alpha=.05)
gg1
gg1$ci
gg2
gg2$ci
...
The slot \texttt{pp} in \texttt{gg1} and \texttt{gg4}
...
d4 <- findgenes(gg4,x[,c(-6,-12)],groups,fdrmax=.05,parametric=TRUE)
table(d1$d,d2$d)
table(d1$d,d3$d)
table(d1$d,d4$d)
@
