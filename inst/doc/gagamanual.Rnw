...
\title{Manual for the R \texttt{gaga} library}
\author{David Rossell \\
\small{Department of Biostatistics} \\
\small{The University of Texas M. D. Anderson Cancer Center} \\
\small{Houston, TX.} \\
...
from the R library \texttt{affy} \cite{rpkg:affy} 
...
for $\nu$.
Both models are fit using the routine \texttt{fit.gg}: the argument
...
estimates obtained from the Armstrong dataset \cite{armstrong:2002},
as described in \cite{rossell:2007}.
...
The object \texttt{xsim} is a list with 3 components.
\texttt{x} is a matrix containing the expression levels for the 100 genes
and 12 samples. 
\texttt{a} is a matrix containing the gene-specific $\alpha$ parameters
(\texttt{a[,1]} contains parameters for the first group,
\texttt{a[,2]} for the second),
and \texttt{l} contains the gene-specific means $\lambda$.
...
Figure \ref{fig:fig1}(b) plots the simulated $(\alpha,\lambda)$
pairs.
...
\section{Model fit}
\label{sec:modelfit}
To fit the model we use the function \texttt{fit.gg}.
First we need to specify a vector \texttt{groups} that
indicates the group to which each sample belongs to.
Second, we need to specify the gene expression patterns
...
\begin{description}
\item Pattern 0: GROUP 1 = GROUP 2 = GROUP 3
\item Pattern 1: GROUP 1 $\neq$ GROUP 2 = GROUP 3
\item Pattern 2: GROUP 1 = GROUP 2 $\neq$ GROUP 3
\item Pattern 3: GROUP 1 $\neq$ GROUP 2 $\neq$ GROUP 3
\end{description}
...
[4,]    0    1    2
\end{verbatim}
That is, the second row indicates that under the second hypothesis groups
2 and 3 are equal, since they both have a \texttt{1} in their entries.
The last row indicates that they are all different by
specifying a different number in each entry.

Now, to fit the GaGa model to our simulated data we use \texttt{fit.gg},
...
from being printed. By default the routine performs \texttt{B=1000} MCMC
iterations, which is typically enough for a GaGa model and it can be done
reasonably fast. 
...
The component \texttt{mcmc} of \texttt{ggfit} contains an object of
type \texttt{mcmc}, as defined in the library \texttt{coda}.
Therefore, simply typing \texttt{plot(ggfit\$mcmc)} produces the
plots in Figure \ref{fig:fig3} (only first plot shown).
For more formal convergence diagnostics, see the documentation of the library \texttt{coda}.
We see that convergence was reached very quickly.
...
Note that they all include the true value.
...
As an alternative, one can estimate the hyper-parameters via
maximum likelihood. This is achieved by specifying \texttt{method='EBayes'}
when calling \texttt{fit.gg}.
Fitting the model via maximum likelihood is usually quicker, and in our
experience it delivers similar results to the Bayesian fit.
...
The parameter estimates are indeed similar to those obtained
from the fully Bayesian fit.
The slot \texttt{pp} in \texttt{ggfit} and \texttt{ggfit.eb}
contains a matrix with the posterior probability
of each expression pattern for each gene.
For example, the first gene has a posterior probability of 0.993
of following pattern 0 ({\it i.e.} being equally expressed)
and a probability of 0.033 of following pattern 1 
({\it i.e.} being differentially expressed) under the fully
Bayesian fit, and 0.034 under the maximum likelihood fit.
...
observed data in Figure \ref{fig:fig3}(a).
Figure \ref{fig:fig3}(b)-(d) compares draws from the posterior 
...
\texttt{sim.gg} and setting the hyper-parameters to their posterior mean.
...
(for details see \cite{rossell:2007}).
...
The parametric version declares 4 genes to be DE, all of them
true positives. It fails to find one of the DE genes.
The non-parametric lists 5 genes as DE, one of them being
a false positive. It fails to find one of the DE genes.
To obtain an estimated frequentist FDR for each
Bayesian FDR one can plot \texttt{d.nonpar\$fdrest}.
